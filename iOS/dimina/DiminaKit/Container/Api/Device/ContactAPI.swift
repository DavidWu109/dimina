...
public class ContactAPI: DMPContainerApi {
    ...
    public override init(app: DMPApp? = nil) {
        super.init(app: app)
        register("chooseContact", handler: chooseContact)
        ...
        DispatchQueue.main.async {
            ...
            ContactPickerDelegate.shared.completion = { contact in
                ...
                if let contact = contact {
                    ...
                    result.set("errMsg", "chooseContact:ok")

                    DMPContainerApi.invokeSuccess(callback: callback, param: result)
                } else {
                    result.set("errMsg", "chooseContact:fail cancel")
        ...
        if authStatus != .authorized {
            CNContactStore().requestAccess(for: .contacts) { granted, error in
                if granted {
                    ...
                    DispatchQueue.main.async {
                        let result = DMPMap()
                        result.set("errMsg", "addPhoneContact:fail auth denied")
    ...
    private static func createContact(param: DMPMap, callback: DMPBridgeCallback?) {
        ...
        guard let firstName = param.getString(key: "firstName") else {
            let result = DMPMap()
            result.set("errMsg", "addPhoneContact:fail missing required fields")
...
//        }
        ...
        do {
            ...
            let result = DMPMap()
            result.set("errMsg", "addPhoneContact:ok")
            DMPContainerApi.invokeSuccess(callback: callback, param: result)
        } catch {
            let result = DMPMap()
            result.set("errMsg", "addPhoneContact:fail \(error.localizedDescription)")
