...
- iOS 暴露小程序渲染、页面显示与隐藏生命周期事件，供宿主统一上报 UBT。
- iOS 新增定位、系统设置等权限桥接能力，并由宿主控制权限确认界面。

### 优化

- iOS 将小程序资源准备与首屏挂载拆分，在宿主转场完成后原子替换根容器，避免首次启动失败和白屏。
- 移除不再使用的小程序冷热启动实例查询接口。

## [v1.4.5] 2026-07-28

### 修复

- 修复直接进入非首页时左侧关闭按钮与 Home 按钮同时显示的问题；根页面不再显示无上一页语义的左侧按钮，退出能力继续由右侧胶囊提供。

## [v1.4.4] 2026-07-27

### 修复

- 恢复 iOS 小程序二级页的系统侧滑返回，并在交互返回完成后同步页面栈与生命周期。
- 小程序根页继续禁止侧滑，关闭小程序后恢复宿主原有手势状态。

## [v1.4.3] 2026-07-27

### 新增

- iOS 新增用户截屏事件支持，并同步前端 API 定义。
- iOS 支持由宿主提供小程序返回、关闭和首页导航控件。

### 优化

- 导航按钮状态由 Dimina 根据页面栈与入口路由统一计算，宿主只负责组件和视觉样式。

## [v1.4.2] 2026-07-27

### 新增

- iOS 新增 QDMP 开发预览客户端，支持带哈希文件清单、版本长轮询和编译快照原子安装。
- 纯样式变更只替换小程序 CSS 并刷新当前 WebView 的样式链接，保留页面栈、数据、表单和滚动状态。

### 优化

- 完整开发更新会重新加载 Service 和资源，同时保持当前页面 path/query；tab 页面优先恢复当前选中的 tab。
- 开发预览客户端限制同源下载并校验 SHA-256，失败时继续使用上一版资源。

### 修复

- 修复 iOS 同步 Bridge 未把 `DMPMap` 序列化为普通对象的问题，确保系统信息、安全区和胶囊几何可正确返回 JavaScript。
